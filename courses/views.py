...
class ContentCreateUpdateView(TemplateResponseMixin, View):
    ...
    def dispatch(self, request, module_id, model_name, id=None):
        self.module = get_object_or_404(Module, id=module_id,
                                        course__owner=TeacherProfile.objects.get(user=request.user))
...
class ContentDeleteView(View):
    ...
    def post(self, request, id):
        content = get_object_or_404(Content, id=id,
                                    module__course__owner=TeacherProfile.objects.get(user=self.request.user))
...
class ModuleContentListView(TemplateResponseMixin, View):
    ...
    def get(self, request, module_id):
        module = get_object_or_404(Module, id=module_id,
                                   course__owner=TeacherProfile.objects.get(user=self.request.user))
        return self.render_to_response({
            'module': module
        })
